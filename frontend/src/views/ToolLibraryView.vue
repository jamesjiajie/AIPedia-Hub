...
const hasFilters = computed(() => Boolean(query.value || category.value || tag.value || favoriteOnly.value || status.value !== 'active'))
const selectedCategoryName = computed(() => {
  if (category.value === '__uncategorized__') return '未分类'
  if (tag.value) return directoryTags.value.find((item) => item.name === tag.value)?.name || tag.value
  return categories.value.find((item) => item.slug === category.value)?.name || '全部工具'
})
...
watch([query, category, tag, favoriteOnly, status, sort], scheduleLoad)
  ...
  <section class="page library-page">
    <div class="library-heading">
      <div>
        ...
        <div class="library-title-row"><h1>工具库</h1><span>{{ total }} 个工具</span></div>
    ...
    <div class="library-layout">
      <aside class="category-directory" aria-label="工具分类目录">
        ...
        <details class="directory-management">
      ...
      <div class="library-content">
        <section v-if="showingClassification" class="classification-panel">
